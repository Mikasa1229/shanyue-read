...
- 旧的 `26893/399953` 是历史规则图规模，只作为历史记录，不代表当前模型证据图。清理后按新抽取器完成前 100 章五轮迭代，最终为 `94` 节点、`31` 边、`19` 条伏笔和 `1` 个跨章事件；核心关系召回 `80%`、真实查询 Recall@5 `100%`、MRR `0.6067`、越界词命中 `0`。详情见 `剑来前100章RAG五轮迭代报告.md`。
...
《剑来》的公开知识基准由 `剑来公开知识基准.md` 固定维护，机器可读 gold 位于
